...
current segment fills, and — for the multi-cardinality unbounded shapes
(any arm with a multi producer or multi consumer) — DEBRA epoch-based
reclamation to free retired segments safely under contention.
...
only one thread races for that cursor. The contended (multi) sides use a
CAS loop and are lock-free.
...
[Unbounded Queue (Queue)](../api/queue.md).
...
detect at runtime that you are temporarily violating the contract.

### Why MPMC is more expensive than SPSC

Every additional contended side adds a CAS loop and a per-slot sequence
counter. The SPSC shape writes the item, then advances a single tail
cursor with a release store — no CAS. The MPMC shape has to publish the
item with a release store, advance the tail with a CAS that may lose to
other producers, and do the same dance on the consumer side. Under
contention, retries multiply.
...
modulo `N` (or `N + 1` for the SPSC shape, which uses one extra slot to
...
rollover for the multi-producer / multi-consumer bounded shapes is
...
it is empty, then advance to the next segment and (for the multi-cardinality
shapes) hand the retired segment to DEBRA for safe reclamation.
