## UnboundedSpmc throughput bench (v5.0.0 3.3.9-D split).
##
...
##
## Covers UnboundedSpmc (single producer + N consumers, DEBRA epoch
...
const
  ...
  UnboundedSpmcMessageCount* {.intdefine.} = 500_000
...
when defined(BenchUnboundedTestCompileTime):
  static:
    doAssert UnboundedSpmcRuns == 3
    doAssert UnboundedSpmcMessageCount == 500_000
...
when defined(benchProgress):
  ...
  # `benchShape` is written by `runUSpmcShape` before any worker thread
...
when isMainModule:
  ...
  echo "UnboundedSpmc Throughput Benchmark"
  echo "===================================="
  echo ""
