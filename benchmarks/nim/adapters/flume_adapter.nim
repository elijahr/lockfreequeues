...
## `libbench_ffi_comparison` cdylib (renamed from
## `libbench_ffi_crossbeam` when flume + kanal were folded into the
## same shared object). The cdylib exports eight `extern "C"` fns
## under the `flume_` prefix:
##
...
## benchmarks/rust/comparison/Cargo.toml`) and then compile the bench
## binary with the link flags emitted by `crossbeam_link.nim` (which
## owns `{.passL.}` for the consolidated cdylib).

when defined(adapter_flume_available):
  import ../bench_common
  import ../adapter
  # Owns `-L<dir> -lbench_ffi_comparison`. Idempotent across multiple
