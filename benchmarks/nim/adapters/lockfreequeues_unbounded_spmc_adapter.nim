## Adapter for lockfreequeues UnboundedSpmc-equivalent (unbounded
...
type
  ...
  LockfreequeuesUnboundedSpmcAdapter*[S: static int, T;
    ...
    ## Mirrors the lockfreequeues_unbounded_mpsc_adapter heap-storage
    ## pattern.
    ##
    ...
    queue*: ptr UnboundedSpmcAdapterQueue[S, T, MaxThreads]
...
proc makeLockfreequeuesUnboundedSpmcAdapter*[S: static int, T;
  ...
  #  * `queueValueInitOk` — set after `newUnboundedSpmcQueue` returns,
  ...
  try:
    ...
    result.queue = create(UnboundedSpmcAdapterQueue[S, T, MaxThreads])
