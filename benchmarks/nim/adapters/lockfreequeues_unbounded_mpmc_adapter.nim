## Adapter for lockfreequeues UnboundedMpmc-equivalent (unbounded MPMC).
##
...
# lockfreequeues_unbounded_spmc_adapter import block.
...
type
  ...
  LockfreequeuesUnboundedMpmcAdapter*[S: static int, T;
    ...
    ## lockfreequeues_unbounded_mpsc_adapter heap-storage pattern.
    ...
    queue*: ptr UnboundedMpmcAdapterQueue[S, T, MaxThreads]
...
proc makeLockfreequeuesUnboundedMpmcAdapter*[S: static int, T;
  ...
  # OOM/other from `newUnboundedMpmcQueue`, or
  ...
  #  * `queueValueInitOk` — set after `newUnboundedMpmcQueue` returns,
  ...
  # This mirrors mpsc's discipline (skip `reset` on a moved-from but
  ...
  try:
    ...
    if not queueInitOk:
      ...
      if result.queue != nil:
        ...
        # never re-assigned — skip `reset` per mpsc's discipline and
