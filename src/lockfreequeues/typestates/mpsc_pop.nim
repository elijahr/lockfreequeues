...
type
  ...
  MPSCPopPointersLoaded*[N: static int] = object ## Loaded head and reservedTail.
  ...
  MPSCPopSlotReady*[N: static int] = object ## Slot is committed - safe to read.
    head*: WrappedValueN[N]
    slot*: PhysicalSlotN[N]
...
typestate MPSCPopOp[N: static int]:
  ...
  states MPSCPopStart[N],
    MPSCPopPointersLoaded[N], MPSCPopNotEmpty[N], MPSCPopSlotReady[N], MPSCPopEmpty[N]
  transitions:
    MPSCPopStart[N] -> MPSCPopPointersLoaded[N]
    MPSCPopPointersLoaded[N] -> MPSCPopNotEmpty[N] | MPSCPopEmpty[N] as MPSCEmptyCheck[
      N
    ]
...
proc checkCommitted*[N, P: static int, T](
    op: MPSCPopNotEmpty[N], queue: var MupsicBase[N, P, T]
...
proc complete*[N, P: static int, T](
    op: MPSCPopSlotReady[N], queue: var MupsicBase[N, P, T]
): T {.inline.} =
