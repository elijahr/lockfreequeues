...
type
  SPSCPopStart*[N: static int] = object
    ## Entry point. No data yet.

  SPSCPopPointersLoaded*[N: static int] = object
    ## Loaded head and tail.
    head*: WrappedValueN1[N]
    tail*: WrappedValueN1[N]

  SPSCPopNotEmpty*[N: static int] = object
    ## Confirmed queue has items. MUST complete.
    head*: WrappedValueN1[N]
    slot*: PhysicalSlotN1[N]

  SPSCPopEmpty*[N: static int] = object
...
typestate SPSCPopOp[N: static int]:
  ...
  transitions:
    ...
    SPSCPopPointersLoaded[N] -> SPSCPopNotEmpty[N] | SPSCPopEmpty[N] as SPSCEmptyCheck[N]
