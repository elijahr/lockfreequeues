...
typestate MPMCPushContext[T, S: static int, MT: static int]:
  ...
  states MPMCPushReady[T, S, MT], MPMCPushSegmentLoaded[T, S, MT],
         MPMCPushSegmentFull[T, S, MT], MPMCPushSlotClaimed[T, S, MT],
         MPMCPushItemWritten[T, S, MT], MPMCPushComplete[T, S, MT]
  transitions:
    MPMCPushReady[T, S, MT] -> MPMCPushSegmentLoaded[T, S, MT]
    MPMCPushSegmentLoaded[T, S, MT] -> (MPMCPushSlotClaimed[T, S, MT] | MPMCPushSegmentFull[T, S, MT] | MPMCPushReady[T, S, MT]) as MPMCSlotClaimResult[T, S, MT]
...
proc tryClaimSlot*[T; S, MT: static int](
  ...
  if tail >= S:
    return MPMCSlotClaimResult[T, S, MT] -> MPMCPushSegmentFull[T, S, MT](
  ...
  if loaded.segment.tail.compareExchange(expected, tail + 1, moAcquire, moRelaxed):
    # Won the slot
    return MPMCSlotClaimResult[T, S, MT] -> MPMCPushSlotClaimed[T, S, MT](
      pinnedHandle: loaded.pinnedHandle,
      pinnedEpoch: loaded.pinnedEpoch,
      queue: loaded.queue,
      segment: loaded.segment,
    ...
    return MPMCSlotClaimResult[T, S, MT] -> MPMCPushReady[T, S, MT](
      MPMCPushContext[T, S, MT](
        pinnedHandle: loaded.pinnedHandle,
        pinnedEpoch: loaded.pinnedEpoch,
        queue: loaded.queue))
...
proc allocateNewSegment*[T; S, MT: static int](
  full: sink MPMCPushSegmentFull[T, S, MT],
...
proc writeItem*[T; S, MT: static int](
  claimed: sink MPMCPushSlotClaimed[T, S, MT],
  item: T
