...
type
  ...
  SPMCPopPointersLoaded*[N: static int] = object ## Loaded reservedHead and tail.
    reservedHead*: WrappedValueN[N]
    tail*: WrappedValueN[N]

  SPMCPopNotEmpty*[N: static int] = object ## Confirmed queue has items.
    reservedHead*: WrappedValueN[N]
    slot*: PhysicalSlotN[N]

  SPMCPopSlotReady*[N: static int] = object ## Slot is committed - safe to claim.
    reservedHead*: WrappedValueN[N]
  ...
  SPMCPopEmpty*[N: static int] = object ## Terminal: queue was empty.

typestate SPMCPopOp[N: static int]:
  inheritsFromRootObj = true
  consumeOnTransition = false # Allow values to be passed across case branches
  states SPMCPopStart[N],
    SPMCPopPointersLoaded[N],
    SPMCPopNotEmpty[N],
    SPMCPopSlotReady[N],
    SPMCPopSlotClaimed[N],
    SPMCPopEmpty[N]
  transitions:
    SPMCPopStart[N] -> SPMCPopPointersLoaded[N]
    SPMCPopPointersLoaded[N] -> SPMCPopNotEmpty[N] | SPMCPopEmpty[N] as SPMCEmptyCheck[
      N
    ]
...
proc checkCommitted*[N, C: static int, T](
    op: SPMCPopNotEmpty[N], queue: var SipmucBase[N, C, T]
  ...
  if not queue.committed.load(op.slot):
    ...
    SPMCCommittedCheck[N] ->
      SPMCPopSlotReady[N](
        reservedHead: op.reservedHead, newReservedHead: newReservedHead, slot: op.slot
      )

proc tryClaim*[N, C: static int, T](
    op: SPMCPopSlotReady[N], queue: var SipmucBase[N, C, T]
): SPMCClaimResult[N] {.inline, transition.} =
  ## CAS to claim the slot. Failure = retry from start.
  var expected = op.reservedHead.value
  if queue.reservedHead.compareExchangeWeak(
    expected, op.newReservedHead.value, moRelease, moAcquire
  ):
    SPMCClaimResult[N] ->
...
proc complete*[N, C: static int, T](
  ...
  discard queue.head.compareExchangeWeak(
    expectedHead, op.newReservedHead.value, moRelease, moAcquire
  )
  value
