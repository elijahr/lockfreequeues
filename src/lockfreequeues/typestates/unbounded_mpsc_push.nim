...
typestate MPSCPushContext[T, S: static int, MT: static int]:
  ...
  states MPSCPushReady[T, S, MT], MPSCPushSegmentLoaded[T, S, MT],
         MPSCPushSegmentFull[T, S, MT], MPSCPushSlotClaimed[T, S, MT],
         MPSCPushItemWritten[T, S, MT], MPSCPushComplete[T, S, MT]
  transitions:
    MPSCPushReady[T, S, MT] -> MPSCPushSegmentLoaded[T, S, MT]
    MPSCPushSegmentLoaded[T, S, MT] -> (MPSCPushSlotClaimed[T, S, MT] | MPSCPushSegmentFull[T, S, MT] | MPSCPushReady[T, S, MT]) as MPSCSlotClaimResult[T, S, MT]
...
proc tryClaimSlot*[T; S, MT: static int](
  ...
  if tail >= S:
    return MPSCSlotClaimResult[T, S, MT] -> MPSCPushSegmentFull[T, S, MT](
  ...
  if loaded.segment.tail.compareExchange(expected, tail + 1, moAcquire, moRelaxed):
    # Won the slot
    return MPSCSlotClaimResult[T, S, MT] -> MPSCPushSlotClaimed[T, S, MT](
      pinnedHandle: loaded.pinnedHandle,
      pinnedEpoch: loaded.pinnedEpoch,
      queue: loaded.queue,
      segment: loaded.segment,
    ...
    return MPSCSlotClaimResult[T, S, MT] -> MPSCPushReady[T, S, MT](
      MPSCPushContext[T, S, MT](
        pinnedHandle: loaded.pinnedHandle,
        pinnedEpoch: loaded.pinnedEpoch,
        queue: loaded.queue))
...
proc allocateNewSegment*[T; S, MT: static int](
  full: sink MPSCPushSegmentFull[T, S, MT],
  ...
  if nextSeg != nil:
    ...
    discard full.queue.tailSegment.compareExchange(expectedSeg, nextSeg, moRelease, moRelaxed)
  else:
    ...
    if full.segment.next.compareExchange(expectedNext, newSegment, moRelease, moRelaxed):
      ...
      var expectedSeg = full.segment
      discard full.queue.tailSegment.compareExchange(expectedSeg, newSegment, moRelease, moRelaxed)
      ...
      if winnerSeg != nil:
        var expectedSeg = full.segment
        discard full.queue.tailSegment.compareExchange(expectedSeg, winnerSeg, moRelease, moRelaxed)
...
proc writeItem*[T; S, MT: static int](
  claimed: sink MPSCPushSlotClaimed[T, S, MT],
  item: T
