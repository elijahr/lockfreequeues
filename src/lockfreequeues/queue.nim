...
export exceptions
# Track E (rkEbr unbounded body) will additionally `import debra` when
# guava's nim-debra 0.8.0 worktree linkage lands.
...
static:
  ...
  doAssert offsetOf(Queue[int, ccSingle, ccSingle, stEager, rkNone,
                           8, 0, 0, 0, 0], head) ==
  ...
  doAssert offsetOf(Queue[int, ccMulti, ccSingle, stEager, rkNone,
                           8, 4, 0, 0, 0], head) ==
  ...
  doAssert offsetOf(Queue[int, ccSingle, ccMulti, stEager, rkNone,
                           8, 0, 4, 0, 0], head) ==
  ...
  doAssert offsetOf(Queue[int, ccMulti, ccMulti, stEager, rkNone,
                           8, 4, 4, 0, 0], head) ==
...
proc initQueue*[T;
                ...
                N, P, C: static int](): Queue[T, ccProd, ccCons, ST, rkNone, N, P, C, 0, 0] =
...
proc getProducer*[T;
                  ...
                  ST: static DeallocationStrategy;
                  N, P, C: static int](
    self: var Queue[T, ccMulti, ccCons, ST, rkNone, N, P, C, 0, 0],
    idx: int = -1
...
proc getConsumer*[T;
                  ...
                  ST: static DeallocationStrategy;
                  N, P, C: static int](
    self: var Queue[T, ccProd, ccMulti, ST, rkNone, N, P, C, 0, 0],
    idx: int = -1
...
when defined(testing):
  ...
  proc checkState*[T;
                   ...
                   N: static int](
      self: var Queue[T, ccSingle, ccSingle, ST, rkNone, N, 0, 0, 0, 0],
      head: int, tail: int, storage: seq[T]) =
  ...
  proc checkState*[T;
    ...
    ## `mupsic.nim` / `sipmuc.nim` / `mupmuc.nim`.
    when ccProd == ccSingle and ccCons == ccSingle:
      {.error: "checkState(uint64) not applicable to SPSC; use the int " &
               "+ seq[T] overload".}
    else:
      check(self.head.load(moRelaxed) == head)
      check(self.tail.load(moRelaxed) == tail)
  ...
  proc checkState*[T;
                   ...
                   N, P, C: static int](
      self: var Queue[T, ccProd, ccCons, ST, rkNone, N, P, C, 0, 0],
      head: uint64, tail: uint64, data: seq[T]) =
    ## Non-SPSC head+tail+data `checkState` — migrated from
    ## `mupsic.nim` / `sipmuc.nim` / `mupmuc.nim`.
    when ccProd == ccSingle and ccCons == ccSingle:
      {.error: "checkState(uint64, seq[T]) not applicable to SPSC; use the " &
               "int + seq[T] overload".}
    else:
      check(self.head.load(moRelaxed) == head)
      check(self.tail.load(moRelaxed) == tail)
