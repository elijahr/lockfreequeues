...
type
  ProducerContext[ST: static DeallocationStrategy, S: static int] = object
    queue: ptr Queue[int, ccSingle, ccMulti, ST, S, MaxThreads]
...
suite "UnboundedSpmc threaded":
  ...
  test "normal segment size":
    var manager = initMultiConsumerManager[MaxThreads]()
    var queue = newUnboundedSpmcQueue[int, stEager, 64, MaxThreads](addr manager)
    var prodCtx =
