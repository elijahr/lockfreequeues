...
suite "Queue MPMC integration":
  ...
  test "wraps":
    check(q.getProducerHere(0).push(@[1, 2, 3, 4, 5, 6, 7, 8]).isNone)

    var popRes = q.getConsumerHere(0).pop(4)

    check(popRes.isSome)
    check(popRes.get == @[1, 2, 3, 4])

    var pushRes = q.getProducerHere(0).push(@[9, 10, 11, 12])

    check(pushRes.isNone)

    q.checkState(head = 4'u64, tail = 12'u64, data = (@[9, 10, 11, 12, 5, 6, 7, 8]))

    popRes = q.getConsumerHere(0).pop(4)
    check(popRes.isSome)
    check(popRes.get == @[5, 6, 7, 8])

    q.checkState(head = 8'u64, tail = 12'u64, data = (@[9, 10, 11, 12, 5, 6, 7, 8]))

    popRes = q.getConsumerHere(1).pop(4)
    check(popRes.isSome)
